<template>
  <div class="container-fluid p-0" :class="{dark : darkMode}" style="overflow: hidden;">
    <div class="d-flex flex-column h-100" style="min-height: inherit;">
      <div class="w-100" v-if="isIncludeRoute">
        <Header />
      </div>
      <router-view />
    </div>
  </div>
</template>

<script>
// import Sidebar from "./components/SidebarComponent.vue";
import Header from "./components/HeaderComponent.vue";
import { EventBus } from "./assets/js/MyEventBus.js";

export default {
  components: {
    // Sidebar,
    Header,
  },
  data() {
    return {
      darkMode: false,
    };
  },
  computed: {
    isIncludeRoute() {
      const excludedRoutes = ['/', '/information/headlist', '/information/articlelist', '/information/albumlist', 'memberList', '/profile', '/memberList', '/event', '/information/history','/familystatistics'];
      return excludedRoutes.includes(this.$route.path);
    }
  },
  created() {
    EventBus.$on("darkMode", (value) => {
      this.darkMode = value;
    });
  },
  watch: {
    // darkMode: {
    //   handler: function () {
    //     console.log(this.darkMode);
    //   },
    // },
  }
};
</script>
 
