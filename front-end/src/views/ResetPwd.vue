...
export default {
  ...
  methods: {
    changePassWord() {
      // Dữ liệu cần mã hóa
      var encryptedPassword = this.encrypt(this.newPwd);
      // Mã hóa dữ liệu với khóa
      var encryptedResetpassword = this.encrypt(this.reNewPwd);

      var decryptPassword = this.decrypt(encryptedPassword);
      // Mã hóa dữ liệu với khóa
      var decryptRepassword = this.decrypt(encryptedResetpassword);
