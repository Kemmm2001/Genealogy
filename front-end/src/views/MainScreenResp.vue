    ...
    <div class="main-grid-container">
        <div class="sidebar" :class="{ expand: expandSidebar }" @mouseover="sidebarHover()" @mouseleave="siderbarLeave()">
            <div class="content-display" style="display: grid; grid-template-rows: 50% 50%; position: relative;">
                ...
                <div v-if="memberRole != 3" class="w-100 d-flex flex-row pt-2">
                    ...
                    <div class="col-6 ps-1 pe-2">
                        <div class="w-100 h-100">
                            <label for="upload" style="width:100%; font-size: 15px; color:white" type="button"
                                class="d-flex align-items-center justify-content-center btn btn-secondary h-100">Xuất dữ
                                liệu vào</label>
                            <input ref="importFile" id="upload" type="file" style="display: none"
                                @change="getFileImportMember($event)" />
                        </div>
                    </div>
                </div>
            ...
            <div class="content-display" style="display: grid; grid-template-rows: 50% 50%;">
                <div class="mems pt-2 pb-1 px-2">
                    <div class="title p-0 d-flex align-items-center justify-content-center"
                        style="background-color: #AED6F1; font-family: 'QuicksandBold'; text-align: center;">Thành viên có
                        trên phả đồ</div>
                    <div class="h-100 w-100" style="overflow-y: auto; background-color: #FFFFFF;">
                        <div v-for="(n, index) in nodes" :key="n.id">
                            <div @click="handleLeftClick(n.id)" @contextmenu.prevent="handleRightClick(n.id)"
                                style="cursor: pointer;"
                                :class="{ 'list-item': true, 'selected-list': n.id == CurrentIdMember, 'ancestor-member': index === 0, 'ellipsis-text': true, 'w-100': true }">
                                {{ n.name }}</div>
                        </div>
                ...
                <div class="mems pt-1 pb-2 px-2">
                    <div class="title d-flex p-0 justify-content-center"
                        style="background-color: #AED6F1; font-family: 'QuicksandBold'; text-align: center; position: relative;">
                        <div class="d-flex align-items-center">Thành viên không có trên phả đồ</div>
                        <div v-if="memberRole != 3" class="d-flex align-items-center h-100 ps-1" style="cursor:pointer"
                            @click="openMemberModal('AddNormal', ' thành viên không có trên phả đồ')">
                    ...
                    <div class="h-100 w-100" style="overflow-y: auto; background-color: #FFFFFF;">
                        <div v-if="ListUnspecifiedMembers" class="d-flex flex-column w-100"
                            style="overflow-y: auto; flex-grow: 1; cursor: pointer">
                            <div v-for="list in ListUnspecifiedMembers" :key="list.id"
                                @click="handleLeftClickUnspecifiedMembers(list.MemberID)"
                                class="list-item ellipsis-text w-100">{{ list.MemberName }}</div>
                        </div>
                        <div class="center-content" v-if="nodes.length == 0">Danh sách chưa có thành viên</div>
                    </div>
                </div>
        ...
        <div class="content">
            ...
            <div v-show="nodes.length == 0" class="center-content">
                <div @click="openMemberModal('AddFirst', 'cụ tổ')"
                    class="btn bg-primary text-white d-flex flex-row align-items-center">
                    <div style="padding-right: 8px;">Thêm thông tin cụ tổ</div>
                    <svg style="fill: white;" class="add-member-icon" xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 448 512">
                        <path
                            d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z" />
                    </svg>
            ...
            <div :class="{ filterExpanded: advancedFilterDown }"
                class="advanced-filter-container d-flex flex-column p-1 position-absolute col-3 col-sm-2 col-md-3 col-lg-4"
                style="cursor: pointer;">
                <div :class="{ expand: advancedFilterDown }" class="filter-item">
                    <div v-if="advancedFilterDown" class="px-2" style="padding-top: 8px;">
                        <select v-model="selectCity" class="d-flex text-center form-select dropdown p-0"
                            @change="getListDistrict(), GetListFilterMember()">
                    ...
                    <div v-if="advancedFilterDown" class="px-2" style="padding-top: 8px;">
                        <select v-model="selectDistrict" class="d-flex text-center form-select dropdown p-0"
                            ...
                            <option v-for="d in ListDistrict" :key="d.id" :value="d.DistrictName">{{ d.DistrictName }}
                            </option>
                        </select>
                    </div>
                    <div v-if="advancedFilterDown" class="px-2" style="padding-top: 8px;">
                        <select v-model="selectBloodType" class="d-flex text-center form-select dropdown p-0"
                            @change="GetListFilterMember()">
                            <option v-for="blood in ListBloodTypeGroup" :key="blood.id" class="dropdown-item"
                                :value="blood.id">{{ blood.BloodType }}</option>
                        </select>
                    </div>
                    <div v-if="advancedFilterDown" class="px-2" style="padding-top: 8px;">
                        <select class="d-flex text-center form-select dropdown p-0" v-model="selectAge"
                            @change="GetListFilterMember()">
                ...
                <div @click="advancedFilterDown = !advancedFilterDown"
                    class="d-flex flex-column align-items-center justify-content-center h-100">
                    <div class="filter-text" v-if="!advancedFilterDown" style="color: white; font-family: 'QuicksandBold'">
                        Bộ lọc nâng cao</div>
                    <div>
                        <svg :class="{ rotateDown: !advancedFilterDown }" class="advanced-filter-icon"
                ...
                <div class="mdl-container">
                    ...
                    <div class="mdl-body" style="overflow-x: hidden;">
                        <div class="select-opts-body list-group">
                            ...
                            <div class="list-group-item" @click="openMemberModal('AddParent', 'phụ huynh')">Thêm phụ huynh
                            </div>
                            <div class="list-group-item" @click="openMemberModal('AddMarriage', 'hôn nhân')">Thêm hôn nhân
                            </div>
                            <div v-for="list in ListMarriedMember" :key="list.id" class="list-group-item"
                                @click="openMemberModal('AddChild', 'Con', list.id)">Thêm Con với {{ list.name }}</div>
                            <div class="list-group-item" @click="openMemberModal('AddChild', 'Con')">Thêm Con</div>
                            <div class="list-group-item" @click="openModalAddMemberFromList()">Thêm mối quan hệ từ Danh Sách
                            </div>
                            <div class="list-group-item" @click="openCfDelModal('removeMember', null, TitleModal)">Xóa thành
                                viên (*)</div>
                            <div class="list-group-item feature-overview">Các chức năng Khác</div>
                            <div class="list-group-item" @click="openCfDelModal('SetPaternalAncestor', null, TitleModal)">
                                Set làm cụ tổ</div>
                            <div v-if="parentRelationship">
                                <div v-for="list in parentRelationship" :key="list.id"
                                    @click="openCfDelModal('LinkRelationship', list.id, list.name)" class="list-group-item">
                                    Nối mối quan hệ với {{ list.name }}</div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mdl-container">
                            ...
                            <table class="table table-member headlist-list m-0">
                                ...
                                <tbody v-if="ResultRelationship">
                                    <tr v-if="ResultRelationship.Father" class="headlist-item headlist-table-item"
                                        @click="getInforMember(ResultRelationship.Father.MemberID)">
                                        <td style="text-align: center;">{{ ResultRelationship.Father.MemberName }}</td>
                                        <td style="text-align: center;">{{ ResultRelationship.Father.Male == 1 ? "Nam" :
                                            "Nữ" }}</td>
                                        <td style="text-align: center;">{{ formatDate(ResultRelationship.Father.Dob) }}</td>
                                        <td style="text-align: center;">Cha</td>
                                        <td style="text-align: center;">
                                            <button class="btn btn-secondary"
                                                @click.stop="openCfDelModal('removeRelationship', ResultRelationship.Father.MemberID, ResultRelationship.Father.MemberName, 'RemoveParent')">Hủy
                                                mối quan hệ</button>
                                        </td>
                                    </tr>
                                    <tr v-if="ResultRelationship.Mother" class="headlist-item headlist-table-item"
                                        @click="getInforMember(ResultRelationship.Mother.MemberID)">
                                        <td style="text-align: center;">{{ ResultRelationship.Mother.MemberName }}</td>
                                        <td style="text-align: center;">{{ ResultRelationship.Mother.Male == 1 ? "Nam" :
                                            "Nữ" }}</td>
                                        <td style="text-align: center;">{{ formatDate(ResultRelationship.Mother.Dob) }}</td>
                                        <td style="text-align: center;">Mẹ</td>
                                        <td style="text-align: center;">
                                            <button class="btn btn-secondary"
                                                @click.stop="openCfDelModal('removeRelationship', ResultRelationship.Mother.MemberID, ResultRelationship.Mother.MemberName, 'RemoveParent')">Hủy
                                                mối quan hệ</button>
                                        </td>
                                    </tr>
                                    <tr v-for="hus in ResultRelationship.Husband" :key="hus.MemberID"
                                        class="headlist-item headlist-table-item"
                                        @click="getInforMember(ResultRelationship.Husband.MemberID)">
                                        ...
                                        <td style="text-align: center;">
                                            <button class="btn btn-secondary"
                                                @click.stop="openCfDelModal('removeRelationship', hus.MemberID, hus.MemberName, 'RemoveMarried')">Hủy
                                                mối quan hệ</button>
                                        </td>
                                    </tr>
                                    <tr v-for="Wife in ResultRelationship.Wife" :key="Wife.MemberID"
                                        class="headlist-item headlist-table-item"
                                        @click="getInforMember(ResultRelationship.Wife.MemberID)">
                                        ...
                                        <td style="text-align: center;">
                                            <button class="btn btn-secondary"
                                                @click.stop="openCfDelModal('removeRelationship', Wife.MemberID, Wife.MemberName, 'RemoveMarried')">Hủy
                                                mối quan hệ</button>
                                        </td>
                                    </tr>
                                    <tr v-for="(c, index) in ResultRelationship.child" :key="index"
                                        class="headlist-item headlist-table-item" @click="getInforMember(c.MemberID)">
                                        ...
                                        <td style="text-align: center;">
                                            <button class="btn btn-secondary"
                                                @click.stop="openCfDelModal('removeRelationship', c.MemberID, c.MemberName, 'RemoveChild')">Hủy
                                                mối quan hệ</button>
                                        </td>
                                    </tr>
                                </tbody>
                    ...
                    <div class="mdl-footer">
                        <div class="h-100 d-flex align-items-center justify-content-end">
                            <div class="pe-2">
                                <button @click="closeModalRelationship()" type="button"
                                    class="btn btn-secondary mt-0">Đóng</button>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="mdl-body">
                        <div class="d-flex flex-row position-relative h-100" style="overflow: hidden;">
                            <div class="col-3 h-100 d-flex flex-column"
                                style="display: grid; grid-template-rows: 8% 8% 8% 8% 68%;">
                                <div class="position-relative d-flex">
                                    <label for="text-search" style="position: absolute; inset: 12px;">
                                        <svg class="text-search-icon" xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 512 512">
                                            <path
                                    ...
                                    <div class="w-100" style="padding: 8px;">
                                        <input v-model="searchKeyword" class="form-control px-5 py-0 w-100 h-100"
                                            id="text-search" type="text" @change="searchMember()" />
                                    </div>
                                </div>
                                <div class="position-relative d-flex">
                                    <div v-if="numberDeath != nodes.length" class="d-flex flex-row align-items-center p-2">
                                        <input v-model="checkAll" type="checkbox" class="m-0 form-check-input"
                                ...
                                <div class="position-relative d-flex">
                                    <div v-if="numberDeath != nodes.length" class="d-flex flex-row align-items-center p-2">
                                        <input v-model="checkWithFilter" type="checkbox" class="m-0 form-check-input"
                                            @change="toggleSelectWithFilter()" />
                                ...
                                <div class="h-100" style="overflow-y: auto;">
                                    <div class="d-flex align-items-center justify-content-center" style="height: 10%"
                                        v-for="(n, index) in ListMemberCanSendMessage" :key="n.id">
                                        <div v-if="n.isDead != 1" :tabindex="index"
                                            class="send-noti-mdl-member d-flex flex-row align-items-center px-2"
                                            :class="{ chosen: ListPhoneToSendMessage.includes(n.MemberID) }"
                                            @click="toggleSelection(n.MemberID)">
                                            <div>
                                                <svg class="send-noti-mdl-member-ava" xmlns="http://www.w3.org/2000/svg"
                                                    viewBox="0 0 448 512">
                                                    <path
                            ...
                            <div class="col-9 h-100 position-relative" style="background: #ebebeb;">
                                <div class="position-absolute w-100 d-flex flex-column h-100" style="top: 0; height: 90%;">
                                    <div class="d-flex flex-row" style="background-color: #FFFFFF; height: 8%;">
                                        <div @click="selectSMS()" :class="{ notiSelected: smsSelected }"
                                            ...
                                            style="border-radius: 0 0.375rem 0 0; cursor: pointer;">SMS</div>
                                        <div @click="selectEmail()" :class="{ notiSelected: emailSelected }"
                                            class="col-6 d-flex align-items-center justify-content-center"
                                            style="border-radius: 0.375rem 0 0 0; cursor: pointer;">Email</div>
                                    </div>
                                    <div class="pt-2" style="height: 92%;">
                                        <div style="overflow-y: auto; height: 90%;">
                                            <div v-if="emailSelected" class="d-flex flex-column">
                                                <div style="cursor: pointer;" v-for="e in ListHistoryEmail" :key="e.id"
                                                    class="sent-mail d-flex flex-row" @click="getInforEmail(e)">
                                                    <div class="col-3 d-flex align-items-center ellipsis-text"
                                                        style="padding-left: 8px; margin: auto;">Chủ đề: {{ e.EmailSubject
                                                        }}
                                                    </div>
                                                    ...
                                                    <div class="col-3 d-flex align-items-center"
                                                        style="justify-content: end; padding-right: 8px;">{{
                                                            formatDate(e.EmailDate) }}</div>
                                                </div>
                                            </div>
                                            <div v-if="smsSelected" class="d-flex flex-column">
                                                <div v-for="m in ListMessage" :key="m.id"
                                                    class="position-relative d-flex flex-row justify-content-end">
                                ...
                                <div class="position-absolute w-100" style="bottom: 0">
                                    <div v-if="smsSelected" class="w-100 px-3 py-2 d-flex flex-row">
                                        <div style="flex-grow: 1;">
                                            <input type="text" class="h-100 w-100 form-control" placeholder="..."
                                                v-model="contentMessage" />
                                        </div>
                                        <div class="d-flex align-items-center ps-2" style="cursor: pointer;"
                                            @click="sendMessageToMember()">
                                            <svg class="noti-send-icon" xmlns="http://www.w3.org/2000/svg"
                                                viewBox="0 0 512 512">
                                                <path
                                    ...
                                    <div v-if="emailSelected" class="w-100 btn px-3 d-flex justify-content-end">
                                        <div @click="expandCreateEmail = !expandCreateEmail"
                                            class="btn btn-primary px-2 py-1">Tạo
                                            email mới</div>
                                    </div>
                                </div>
                                <div class="position-absolute create-mail" :class="{ expanded: expandCreateEmail }">
                                    <div class="w-100 h-100 d-flex flex-column">
                                        <div
                                            class="create-mail-title d-flex align-items-center justify-content-center position-relative">
                                            <div>Email mới</div>
                                            <div class="create-mail-close position-absolute"
                                                @click="expandCreateEmail = !expandCreateEmail">
                                                <div class="position-relative h-100 w-100">
                                                    <svg class="create-mail-close-icon position-absolute"
                                        ...
                                        <div class="create-mail-topic px-2 w-100">
                                            <input v-model="subjectEmail" type="text" class="mail-topic w-100 p-2"
                                                placeholder="Chủ đề email" />
                                        </div>
                                        <div class="create-mail-content px-2 w-100">
                                            <textarea v-model="contentEmail"
                                                style="resize: none; outline: none; border: none;" class="h-100 w-100 p-2"
                                                placeholder="Viết gì đó..."></textarea>
                                        </div>
                                        <div class="create-mail-footer d-flex flex-row px-3 py-2 w-100"
                                            style="justify-content: end;">
                                            <div class="d-flex flex-row" @click="sendEmailToMember()">
                                                <div style="border-radius: 50% 0 0 50%; background: #007bff; width: 25px;">
                                                </div>
                                                <div class="btn d-flex align-items-center justify-content-center"
                                                    ...
                                                    Gửi</div>
                                                <div style="border-radius: 0 50% 50% 0; background: #007bff; width: 25px;">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div v-if="inforEmail" class="col-9 email-detail position-absolute" style="bottom: 0; right: 0;"
                                :class="{ expand: emailDetail }">
                                <div
                                    class="create-mail-title d-flex align-items-center justify-content-center position-relative">
        ...
        <div class="relation-container">
            <modal name="relation-mdl">
                <div class="mdl-container">
                    <div class="mdl-title">
                        <div style="font-family: 'QuicksandBold'; font-size: 17px;">Xác định mối quan hệ giữa 2 thành viên
                        </div>
                        ...
                        <div class="relation-body" style="flex-grow: 1; background-color: #FFFFFF;">
                            <div class="col-6">
                                <div class="h-100 d-flex flex-column p-2"
                                    style="background-color: #FFFFFF; border-radius: 0.375rem;">
                                    <div class="d-flex flex-row pb-2">
                                        ...
                                        <div class="d-flex flex-column" style="flex-grow: 1; padding-left: 8px;">
                                            <input type="text" class="compare-modal-item form-control"
                                                :value="objCompareMember1.name" disabled />
                                            <div class="d-flex flex-row pt-2">
                                                <div class="col-6" style="padding-right: 4px">
                                                    <input type="text" class="compare-modal-item form-control"
                                                        :value="objCompareMember1.gender" disabled />
                                                </div>
                                                <div class="col-6" style="padding-left: 4px">
                                                    <input type="text" class="compare-modal-item form-control"
                                                        :value="'Đời ' + objCompareMember1.generation" disabled />
                                                </div>
                                            </div>
                                            <div class="pt-2">
                                                <input type="text" class="compare-modal-item form-control"
                                                    :value="'Ngày Sinh: ' + (objCompareMember1.dob ? objCompareMember1.dob : 'không rõ')"
                                                    disabled />
                                            </div>
                                            <div class="pt-2">
                                                <input type="text" class="compare-modal-item form-control"
                                                    :value="'Ngày Mất: ' + (objCompareMember1.dod ? objCompareMember1.dod : 'không rõ')"
                                                    disabled />
                                            </div>
                                        </div>
                                    </div>
                                    <div class="d-flex flex-column" style="flex-grow: 1; background-color: #f2f2f2;">
                                        <div class="compare-modal-item mx-2 mt-2">
                                            <input type="text" class="w-100 h-100 form-control"
                                                :value="objCompareMember1.father !== undefined ? 'Bố: ' + objCompareMember1.father : 'Bố: Không có trên phả đồ'"
                                                disabled />
                                        </div>
                                        <div class="compare-modal-item mx-2 mt-2">
                                            <input type="text" class="w-100 h-100 form-control"
                                                :value="objCompareMember1.mother !== undefined ? 'Mẹ: ' + objCompareMember1.mother : 'Mẹ: Không có trên phả đồ'"
                                                disabled />
                                        </div>
                                        <div class="compare-modal-item mx-2 mt-2">
                                            <input type="text" class="w-100 h-100 form-control"
                                                :value="resultCompare1 !== undefined ? 'Mối quan hệ: ' + resultCompare1 : 'Không xác định'"
                                                disabled />
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-6">
                                <div class="h-100 d-flex flex-column p-2"
                                    ...
                                    <div class="d-flex flex-row pb-2">
                                        ...
                                        <div class="d-flex flex-column" style="flex-grow: 1; padding-left: 8px;">
                                            <input type="text" class="compare-modal-item form-control"
                                                :value="objCompareMember2.name" disabled />
                                            <div class="d-flex flex-row pt-2">
                                                <div class="col-6" style="padding-right: 4px">
                                                    <input type="text" class="compare-modal-item form-control"
                                                        :value="objCompareMember2.gender" disabled />
                                                </div>
                                                <div class="col-6" style="padding-left: 4px">
                                                    <input type="text" class="compare-modal-item form-control"
                                                        :value="'Đời ' + objCompareMember2.generation" disabled />
                                                </div>
                                            </div>
                                            <div class="pt-2">
                                                <input type="text" class="compare-modal-item form-control"
                                                    :value="'Ngày Sinh: ' + (objCompareMember2.dob ? objCompareMember2.dob : 'không rõ')"
                                                    disabled />
                                            </div>
                                            <div class="pt-2">
                                                <input type="text" class="compare-modal-item form-control"
                                                    :value="'Ngày Mất: ' + (objCompareMember2.dod ? objCompareMember2.dod : 'không rõ')"
                                                    disabled />
                                            </div>
                                        </div>
                                    </div>
                                    <div class="d-flex flex-column pb-2" style="flex-grow: 1; background-color: #f2f2f2;">
                                        <div class="compare-modal-item mx-2 mt-2">
                                            <input type="text" class="w-100 h-100 form-control"
                                                :value="objCompareMember2.father !== undefined ? 'Bố: ' + objCompareMember2.father : 'Không có trên phả đồ'"
                                                disabled />
                                        </div>
                                        <div class="compare-modal-item mx-2 mt-2">
                                            <input type="text" class="w-100 h-100 form-control"
                                                :value="objCompareMember2.mother !== undefined ? 'Mẹ: ' + objCompareMember2.mother : 'Không có trên phả đồ'"
                                                disabled />
                                        </div>
                                        <div class="compare-modal-item mx-2 mt-2">
                                            <input type="text" class="w-100 h-100 form-control"
                                                :value="resultCompare2 !== undefined ? 'Mối quan hệ: ' + resultCompare2 : 'Mối quan hệ: Không xác định'"
                                                disabled />
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="mdl-footer">
                        <div class="h-100 d-flex align-items-center justify-content-end">
                            <div class="pe-2">
                                <button @click="closeCompareModal()" type="button"
                                    class="btn btn-secondary mt-0">Đóng</button>
                            </div>
                        </div>
                    </div>
        ...
        <div class="add-relation-container">
            <modal name="add-relation-mdl">
                <div class="mdl-container">
                    <div class="mdl-title">
                        <div style="font-family: 'QuicksandBold'; font-size: 17px;">Thêm {{ TitleModal }} Từ Danh Sách
                        </div>
                        ...
                        <div class="add-relation-body">
                            <div class="d-flex flex-row">
                                <div class="col-md-4 m-2">
                                    <input type="text" class="form-control modal-item m-0"
                                ...
                                <table class="table table-member headlist-list m-0">
                                    ...
                                    <tbody style="cursor: pointer">
                                        <tr v-for="(m, index) in ListUnspecifiedMembers" :key="m.id"
                                            @click="selectMemberFromTable(m, index)"
                                            ...
                                            <td style="text-align: center;">
                                                <select v-model="m.relationship" class="form-control"
                                                    style="text-align: center; width: 150px; margin: auto">
                    ...
                    <div class="mdl-footer">
                        <div class="h-100 d-flex align-items-center justify-content-end">
                            <div class="pe-2">
                                <button @click="addMemberFromList()" type="button"
                                    class="btn btn-primary mt-0">Thêm</button>
                            </div>
                            <div class="pe-2">
                                <button @click="closeModalAddMemberFromList()" type="button"
                                    class="btn btn-secondary mt-0">Đóng</button>
                            </div>
                        </div>
                    </div>
        ...
        <div class="view-member-container">
            <modal name="view-member-mdl">
                <div class="mdl-container">
                    <div class="mdl-title">
                        <div style="font-family: 'QuicksandBold'; font-size: 17px;">{{ TitleModal }}
                        </div>
                    ...
                    <div class="mdl-body">
                        <div class="view-member-body" style="padding: 0; height: 100%;">
                            <div class="col-2 select-menu">
                                ...
                                <div v-if="isEdit" class="custom-info" :class="{ selected: extendedJob }"
                                    @click="selectedJob()">
                                    <h5>Nghề nghiệp</h5>
                                </div>
                                <div v-if="isEdit" class="custom-info" :class="{ selected: extendedEdu }"
                                    @click="selectedEdu()">
                                    <h5>Giáo dục</h5>
                                </div>
                            ...
                            <div class="col-10 h-100" style="padding-top: 15px" v-if="extendedInfo">
                                <div class="h-100 d-flex flex-row">
                                    <div v-if="isEdit" class="col-4 px-2 d-flex align-items-center justify-content-center"
                                        style="height: 50%;">
                                        <img id="myImage" @load="getImageSize" style="width:100%;"
                                            :class="{ fitHeight: heightLarger }" v-if="avatarSrc" :src="avatarSrc"
                                            alt="Avatar" @click="triggerFileInputClick()" />
                                        <div v-else style="fill: #000000; height: 100%; width: 100%;">
                                            <div @click="triggerFileInputClick()" class="w-100 h-100 position-relative"
                                        ...
                                        <div class="form-group" style="display:none">
                                            <label style="margin-bottom:10px" v-if="objMemberInfor.Image"
                                                for="imageUpload">Thay đổi ảnh</label>
                                            <label style="margin-bottom:10px" v-else for="imageUpload">Tải ảnh lên</label>
                                            <input ref="fileInputRef" type="file" class="form-control input-file"
                                                id="imageUpload" accept="image/*" @change="updateAvatar($event)" />
                                        </div>
                                    </div>
                                    <div class="w-100" style="display: grid; grid-template-rows: 8% 8% 8% 8% 8% 15% 8% 8% 29%">
                                        <div class="pb-2" style="position: relative; margin-right:10px">
                                            <input v-model="objMemberInfor.MemberName" type="text"
                                                class="form-control modal-item" placeholder />
                                            <label class="form-label" for="input"
                                                :class="{ 'active': objMemberInfor.MemberName }">Tên thành viên đầy
                                                đủ</label>
                                        </div>
                                        <div class="pb-2" style="display:flex">
                                            <div style="position: relative; width: 50%;margin-right: 10px;">
                                                <input v-model="objMemberInfor.NickName" type="text"
                                                    class="form-control modal-item" placeholder />
                                                <label class="form-label" for="input"
                                                    :class="{ 'active': objMemberInfor.NickName }">Tên thường gọi</label>
                                            </div>
                                            <div style="position: relative;width: 50%; margin-right: 10px;"
                                                v-if="objMemberInfor.FatherID != null && action == null">
                                                <input v-model="objMemberInfor.BirthOrder" type="number" min="0"
                                                    class="form-control modal-item" placeholder />
                                                <label class="form-label-number" for="input"
                                                    :class="{ 'active': objMemberInfor.BirthOrder }">Con thứ</label>
                                            </div>
                                            <div style="position: relative;width: 50%; margin-right: 10px;"
                                                v-else-if="objMemberInfor.FatherID == null && action == null && objMemberInfor.MarriageNumber != null && objMemberInfor.MotherID == null">
                                                <input v-model="objMemberInfor.MarriageNumber" type="number" min="0"
                                                    max="20" class="form-control modal-item" disabled placeholder />
                                                <label class="form-label-number" for="input"
                                                    :class="{ 'active': objMemberInfor.MarriageNumber }">{{ objMemberInfor.Male
                                                        == 1 ? 'Chồng thứ' : 'Vợ thứ' }}</label>
                                            </div>
                                            <div style="position: relative;width: 50%; margin-right: 10px;"
                                                v-else-if="action == 'AddChild'">
                                                <input v-model="objMemberInfor.BirthOrder" type="number" min="0"
                                                    class="form-control modal-item" placeholder />
                                                <label class="form-label-number" for="input"
                                                    :class="{ 'active': objMemberInfor.BirthOrder }">Con thứ</label>
                                            ...
                                            <div style="position: relative;width: 50%; margin-right: 10px;"
                                                v-else-if="action == 'AddMarriage'">
                                                <input v-model="objMemberInfor.MarriageNumber" type="number" min="0"
                                                    max="20" class="form-control modal-item" disabled placeholder />
                                                <label class="form-label-number" for="input"
                                                    :class="{ 'active': objMemberInfor.MarriageNumber }">{{ isFather == false
                                                        ? 'Chồng thứ' : 'Vợ thứ' }}</label>
                                            </div>
                                            <div style="position: relative;width: 50%; margin-right: 10px;"
                                                v-else-if="action == 'AddParent'">
                                                <input v-model="objMemberInfor.BirthOrder" disabled type="number" min="0"
                                                    class="form-control modal-item" placeholder />
                                                <label class="form-label-number" for="input"
                                                    :class="{ 'active': objMemberInfor.BirthOrder }">Con thứ</label>
                                            </div>
                                            <div style="position: relative;width: 50%; margin-right: 10px;" v-else>
                                                <input v-model="objMemberInfor.BirthOrder" type="number" min="0" max="20"
                                                    class="form-control modal-item" placeholder />
                                                <label class="form-label-number" for="input"
                                                    :class="{ 'active': objMemberInfor.BirthOrder }">Con thứ</label>
                                            </div>
                                        ...
                                        <div class="pb-2" style="display:flex">
                                            <div style="position: relative; width: 50%;margin-right: 10px;">
                                                <select v-model="objMemberInfor.NationalityID"
                                                    class="form-select modal-item">
                                        ...
                                        <div class="pb-2" style="position: relative; margin-right:10px">
                                            <input v-model="objMemberInfor.Origin" type="text"
                                                class="form-control modal-item" placeholder />
                                            <label class="form-label" for="input"
                                            ...
                                            <div class="pb-2" style="height: 60%; display: flex;">
                                                <div style="position: relative; width: 50%;margin-right: 10px;">
                                                    <input v-model="objMemberInfor.Dob" type="date"
                                                        class="form-control modal-item" placeholder
                                                        @change="convertSolarToLunar('live')" />
                                                    <label class="form-label" for="input">Dương Lịch</label>
                                                </div>
                                                <div style="position: relative;width: 50%; margin-right: 10px;">
                                                    <input v-model="objMemberInfor.LunarDob" type="date"
                                                        class="form-control modal-item" placeholder
                                                        @change="convertLunarToSolar('live')" />
                                                    <label class="form-label-number" min="0" for="input">Âm lịch</label>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="pb-2" style="position: relative; margin-right:10px">
                                            <input v-model="objMemberInfor.BirthPlace" type="text"
                                                class="form-control modal-item" placeholder />
                                            <label class="form-label" for="input"
                                                :class="{ 'active': objMemberInfor.BirthPlace }">Nơi sinh</label>
                                        </div>
                                            ...
                                            <div class="pb-2" style="display:flex; height: 28%;">
                                                <div style="position: relative; width: 50%; margin-right: 10px;">
                                                    <input v-model="objMemberInfor.Dod" type="date"
                                                        class="form-control modal-item" placeholder
                                                        @change="convertSolarToLunar('died')" />
                                                    <label class="form-label" for="input">Dương Lịch</label>
                                                </div>
                                                <div style="position: relative;width: 50%; margin-right: 10px;">
                                                    <input v-model="objMemberInfor.LunarDod" type="date"
                                                        class="form-control modal-item" placeholder
                                                        @change="convertLunarToSolar('died')" />
                                                    <label class="form-label-number" min="0" for="input">Âm lịch</label>
                                                </div>
                                            </div>
                                            <div class="pb-2" style="position: relative; margin-right: 10px; height: 26%;">
                                                <input v-model="objMemberInfor.PlaceOfDeadth" type="text"
                                                    class="form-control modal-item" placeholder />
                                                <label class="form-label" for="input">Nơi Mất</label>
                                            </div>
                                            <div class="pb-2" style="position: relative; margin-right: 10px; height: 26%;">
                                                <input v-model="objMemberInfor.GraveSite" type="text"
                                                    class="form-control modal-item" placeholder />
                                ...
                                <div class="row h-100 px-2" style="display: grid; grid-template-rows: 8% 8% 13% 8% 8% 55%;">
                                    ...
                                    <div class="d-flex justify-content-end">
                                        <div class="form-group d-flex align-items-center" role="group">
                                            ...
                                            <button type="button" class="btn btn-info mr-1" @click="updateEducationMember()"
                                                style="margin-right:10px">Sửa</button>
                                            <button type="button" class="btn btn-danger mr-1" @click="removeEducation()"
                                                style="margin-right:10px">Xóa</button>
                                        </div>
                                    </div>
                                    <div class="form-group" style="margin-top: 13px; padding-right: 22px">
                                        <table class="table">
                                            ...
                                            <tbody>
                                                <tr v-for="edu in ListMemberEducation" :key="edu.id"
                                                    @click="selectRowEducation(edu)"
