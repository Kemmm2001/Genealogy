<template>
    <div class="container-fluid" style="overflow-y: auto;">
        <div class="m-5">
            ...
            <div class="d-flex flex-row m-4">
                ...
                <div class="col-2 filter">
                    ...
                    <p class="d-block">Thành viên lớn tuổi nhất còn sống: {{ this.memberOldestAlive.name }}
                        ({{ this.memberOldestAlive.age }})</p>
                </div>
            </div>
        ...
        <div class="m-5" style="min-height: 100vh; ">
            ...
            <div class="m-4">
                <div>
                    <span>* Chọn các ô bên dưới để lọc dữ liệu biểu đồ</span><br>
                ...
                <div class="d-flex row" style="min-width: 100vh;">
                    ...
                    <div class="graph" style="height: 600px;max-width: 2000px;margin-top: 100px;padding-bottom: 100px;flex: 1;border: 1px solid #ccc;">
                        <Bar id="graphMonthDob" :options="graphMonthDob.chartOptions" :data="graphMonthDob.chartData"
                            :style="myMonthStyle" />
...
export default {
    ...
    computed: {
        ...
        graphGender() {
            return {
                chartData: {
                    labels: ["Nam: " + this.numberMaleFilter, "Nữ: " + this.numberFemaleFilter],
                    datasets: [
                        {
                            label: "",
        ...
        graphAlive() {
            ...
            return {
                chartData: {
                    labels: ["Đã mất: " + this.numberDiedFilter, "Còn sống: " + this.numberAliveFilter],
                    datasets: [
                        {
                            label: "",
        ...
        graphAge() {
            return {
                chartData: {
                    labels: ["Từ 0-5 tuổi: " + this.age0to5Filter, "Từ 6-17 tuổi: " + this.age6to17Filter, "Từ 18-40 tuổi: " + this.age18to40Filter, "Từ 41-60 tuổi: " + this.age41to60Filter, "Trên 61 tuổi: " + this.age61upFilter],
                    datasets: [
                        {
                            label: "",
                            data: [this.age0to5Filter, this.age6to17Filter, this.age18to40Filter, this.age41to60Filter, this.age61upFilter],
        ...
        graphMonthDob() {
            return {
                chartData: {
                    labels: ["Tháng 01: "+this.monthDobFilter[0], "Tháng 2: "+this.monthDobFilter[1], "Tháng 3: "+this.monthDobFilter[2], "Tháng 4: "+this.monthDobFilter[3], "Tháng 5: "+this.monthDobFilter[4], "Tháng 6: "+this.monthDobFilter[5], "Tháng 7: "+this.monthDobFilter[6], "Tháng 8: "+this.monthDobFilter[7], "Tháng 9: "+this.monthDobFilter[8], "Tháng 10: "+this.monthDobFilter[9], "Tháng 11: "+this.monthDobFilter[10], "Tháng 12: "+this.monthDobFilter[11]],
                    datasets: [
                        {
                            label: "",
                            data: [this.monthDobFilter[0], this.monthDobFilter[1], this.monthDobFilter[2], this.monthDobFilter[3], this.monthDobFilter[4], this.monthDobFilter[5], this.monthDobFilter[6], this.monthDobFilter[7], this.monthDobFilter[8], this.monthDobFilter[9], this.monthDobFilter[10], this.monthDobFilter[11]],
                            backgroundColor: 'red',
                        }
                        ,]
                },
    ...
    methods: {
        ...
        takeInforList() {
            ...
            for (let i = 0; i < this.memberList.length; i++) {
                ...
                if(this.getNumberMemberByGeneration(this.memberList[i].generation) > max){
                ...
                if (this.ageMember(this.memberList[i].dob) >= 0 && this.ageMember(this.memberList[i].dob) <= 5) {
                    this.age0to5 += 1;
                }
                if (this.ageMember(this.memberList[i].dob) >= 6 && this.ageMember(this.memberList[i].dob) <= 17) {
                    this.age6to17 += 1;
                }
                if (this.ageMember(this.memberList[i].dob) >= 18 && this.ageMember(this.memberList[i].dob) <= 40) {
                    this.age18to40 += 1;
                }
                if (this.ageMember(this.memberList[i].dob) >= 41 && this.ageMember(this.memberList[i].dob) <= 60) {
                    this.age41to60 += 1;
                }
                ...
                let month = 1;
                while(new Date(this.formatDate(this.memberList[i].dob)).getMonth() + 1 != month){
                    month += 1;
                }
                ...
                if (this.ageMember(this.memberList[i].dob) > this.memberOldestAlive.age || this.memberList[i].isDead == 0) {
        ...
        filter() {
            ...
            if (this.ageSearch != "all") {
                if (this.ageSearch == "0-5") {
                    ...
                    this.memberFilter = this.memberFilter.filter(
                        (member) => 0 <= this.ageMember(member.dob) && this.ageMember(member.dob) <= 5
                    );
                }
                if (this.ageSearch == "6-17") {
                    console.log(this.ageSearch)
                    this.memberFilter = this.memberFilter.filter(
                        (member) => 6 <= this.ageMember(member.dob) && this.ageMember(member.dob) <= 17
                    );
                }
                if (this.ageSearch == "18-40") {
                    console.log(this.ageSearch)
                    this.memberFilter = this.memberFilter.filter(
                        (member) => 18 <= this.ageMember(member.dob) && this.ageMember(member.dob) <= 40
                    );
                }
                if (this.ageSearch == "41-60") {
                    console.log(this.ageSearch)
                    this.memberFilter = this.memberFilter.filter(
                        (member) => 41 <= this.ageMember(member.dob) && this.ageMember(member.dob) <= 60
                    );
                }
                if (this.ageSearch == ">60") {
                    console.log(this.ageSearch)
                    this.memberFilter = this.memberFilter.filter(
                        (member) => this.ageMember(member.dob) && this.ageMember(member.dob) > 60
                    );
                }
            }
            ...
            for (let i = 0; i < this.memberFilter.length; i++) {
                ...
                let month = 1;
                while(new Date(this.formatDate(this.memberFilter[i].dob)).getMonth() + 1 != month){
                    month += 1;
                }
                this.monthDobFilter[month-1] += 1;
                if (this.ageMember(this.memberFilter[i].dob) >= 0 && this.ageMember(this.memberFilter[i].dob) <= 5) {
                    this.age0to5Filter += 1;
                }
                if (this.ageMember(this.memberFilter[i].dob) >= 6 && this.ageMember(this.memberFilter[i].dob) <= 17) {
                    this.age6to17Filter += 1;
                }
                if (this.ageMember(this.memberFilter[i].dob) >= 18 && this.ageMember(this.memberFilter[i].dob) <= 40) {
                    this.age18to40Filter += 1;
                }
                if (this.ageMember(this.memberFilter[i].dob) >= 41 && this.ageMember(this.memberFilter[i].dob) <= 60) {
