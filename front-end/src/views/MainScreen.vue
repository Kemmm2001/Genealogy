...
<template>
  <div class="d-flex h-100 w-100 position-relative">
    <div class="list h-100 d-flex flex-column align-items-center">
      <div v-if="memberRole != 3" class="w-100 d-flex flex-column" style="height: 15%;">
        <div class="w-100 h-100 d-flex flex-column" style="align-items: center">
          <div class="w-100 d-flex flex-row" style="padding-top: 8px; height: 50%;">
            <div class="col-6" style="padding-left: 8px; padding-right: 6px">
              <div class="w-100 h-100">
                <button @click="openNotiModal()" style="width:100%; font-size: 14px;" type="button"
                  class="p-0 btn btn-secondary h-100">Tạo thông báo</button>
              </div>
            </div>
            <div class="col-6" style="padding-left: 6px; padding-right: 8px">
              <div class="w-100 h-100">
                <button @click="openCompareModal()" style="width:100%; font-size: 14px;" type="button"
                  :class="{ 'p-0': true, 'btn': true, 'h-100': true, 'btn-secondary': !isCompare, 'btn-primary': isCompare }">Xác
                  định quan hệ</button>
              </div>
            </div>
          </div>
          <div class="w-100 d-flex flex-row" style="padding-top: 8px; height: 50%;">
            <div class="col-6" style="padding-left: 8px; padding-right: 6px">
              <div class="w-100 h-100">
                <button @click="BackUpdata()" style="width:100%; font-size: 14px; color:white" type="button"
                  class="p-0 btn btn-secondary h-100">Lưu trữ dữ liệu</button>
              </div>
            </div>
            <div class="col-6" style="padding-left: 6px; padding-right: 8px">
              <div class="w-100 h-100">
                <label for="upload" style="width:100%; font-size: 14px; color:white" type="button"
                  class="d-flex align-items-center justify-content-center p-0 btn btn-secondary h-100">Xuất dữ liệu
                  vào</label>
      ...
      <div class="w-100 d-flex flex-column px-2" :class="{ height100: memberRole == 3 }"
        style="padding: 12px; min-height: 85%; font-family: 'QuicksandBold', sans-serif;">
        <div class="existing-members d-flex flex-column w-100 position-relative">
          <div class="d-flex align-items-center justify-content-center px-2 py-1 list-title">Thành viên có trên phả đồ
          </div>
          <div class="d-flex flex-column w-100" style="overflow-y: auto; flex-grow: 1; cursor: pointer">
            <div v-for="(n, index) in nodes" :key="n.id">
              <div @click="handleLeftClick(n.id)" @contextmenu.prevent="handleRightClick(n.id)"
                :class="{ 'list-item': true, 'selected-list': n.id == CurrentIdMember, 'ancestor-member': index === 0 }">
                {{ n.name }}</div>
            </div>
          </div>
          <div style="inset: 0; margin: auto; position: absolute; height: fit-content; width: fit-content;"
            v-if="nodes.length == 0">Danh sách chưa có thành viên</div>
        </div>
        <div class="d-flex nonexisting-members flex-column w-100 position-relative" style="margin: 8px 0">
          <div class="d-flex flex-row px-2 py-1 list-title">
            ...
            <div v-if="memberRole != 3" class="d-flex align-items-center justify-content-center"
          ...
          <div v-if="ListUnspecifiedMembers" class="d-flex flex-column w-100"
            ...
            <div v-for="list in ListUnspecifiedMembers" :key="list.id"
              @click="handleLeftClickUnspecifiedMembers(list.MemberID)" class="list-item">{{ list.MemberName }}</div>
          </div>
          <div style="inset: 0; margin: auto; position: absolute; height: fit-content; width: fit-content;"
            v-if="ListUnspecifiedMembers.length == 0">Danh sách chưa có thành viên</div>
        </div>
      </div>
    </div>
        ...
        <div :class="{ expand: advancedFilterDown }" class="filter-item">
          ...
          <div v-if="advancedFilterDown" class="px-2" style="padding-top: 8px;">
            <select class="d-flex text-center form-select dropdown p-0" v-model="selectAge"
              ...
              <option v-for="age in ListAgeGroup" :key="age.id" class="dropdown-item" :value="age.id">{{ age.From != 61 ?
                age.From : 'Trên ' + age.From }} {{ age.End != 200 ? ' - ' + age.End : '' }} Tuổi</option>
            </select>
          </div>
        </div>
        ...
        <div class="card" style="width: 370px;left:45%">
          ...
          <div class="card-body" style="padding: 5px;height:auto">
            <div class="list-group">
              ...
              <div class="list-group-item" @click="openMemberModal('AddChild', 'Con')">Thêm Con</div>
              <div class="list-group-item" @click="openModalAddMemberFromList()">Thêm mối quan hệ từ Danh Sách</div>
              <div class="list-group-item" @click="openCfDelModal('removeMember', null, TitleModal)">Xóa thành viên (*)
              </div>
              <div class="list-group-item feature-overview">Các chức năng Khác</div>
              <div class="list-group-item" @click="openCfDelModal('SetPaternalAncestor', null, TitleModal)">Set làm cụ tổ
              </div>
              <div v-if="parentRelationship">
                <div v-for="list in parentRelationship" :key="list.id"
                  @click="openCfDelModal('LinkRelationship', list.id, list.name)" class="list-group-item">Nối mối quan hệ
                  với {{ list.name }}</div>
              </div>
            </div>
          </div>
        ...
        <div class="form-group h-100">
          <div class="w-100 h-100 add-article-modal">
            <div class="d-flex flex-row w-100 align-items-center position-relative">
              <div class="col-md-12 modal-title d-flex align-items-center justify-content-center w-100">Các mối quan hệ
                của</div>
                ...
                <table class="table table-member headlist-list m-0">
                  ...
                  <tbody v-if="ResultRelationship">
                    ...
                    <tr v-for="Wife in ResultRelationship.Wife" :key="Wife.MemberID"
                      class="headlist-item headlist-table-item" @click="getInforMember(ResultRelationship.Wife.MemberID)">
                    ...
                    <tr v-for="(c, index) in ResultRelationship.child" :key="index"
                      ...
                      <td style="text-align: center;">
                        <button class="btn btn-secondary"
                          @click.stop="openCfDelModal('removeRelationship', c.MemberID, c.MemberName, 'RemoveChild')">Hủy mối
                          quan hệ</button>
                      </td>
                    </tr>
                  </tbody>
    ...
    <modal name="noti-modal">
      <div class="h-100 d-flex flex-column" style="background-color: white;">
        <div class="modal-title d-flex flex-row align-items-center justify-content-center position-relative"
          style="border-radius:0px">
          <div class="col-9 d-flex align-items-center justify-content-center" style="height:50px">Thông báo tới thành viên
          </div>
          ...
          <div class="col-3 h-100 d-flex flex-column" style="overflow-y: auto">
            <div class="position-relative d-flex" style="height: 48px;">
              ...
              <div class="w-100" style="height: 48px; padding: 8px;">
                <input v-model="searchKeyword" class="form-control px-5 py-0 w-100 h-100" id="text-search" type="text"
                  @change="searchMember()" />
              </div>
            </div>
            <div class="position-relative d-flex" style="height: 48px;">
            ...
            <div>
              <div v-for="(n, index) in ListMemberCanSendMessage" :key="n.id">
                <div v-if="n.isDead != 1" :tabindex="index"
                  ...
                  :class="{ chosen: ListPhoneToSendMessage.includes(n.MemberID) }" @click="toggleSelection(n.MemberID)">
                  <div>
          ...
          <div class="col-9 h-100 position-relative" style="background: #ebebeb">
            <div class="position-absolute w-100 d-flex flex-column h-100" style="top: 0;">
              ...
              <div v-if="emailSelected" class="d-flex flex-column mt-2"
                style="height: calc(100% - 116px); overflow-y: auto;">
                <div v-for="e in ListHistoryEmail" :key="e.id" class="sent-mail d-flex flex-row">
                  <div class="col-3 d-flex align-items-center" style="height: 48px; padding-left: 8px">Chủ đề:
                    {{ e.EmailSubject }}</div>
              ...
              <div v-if="smsSelected" class="d-flex flex-column"
                style="height: calc(100% - 108px); align-items: flex-end; padding-top: 6px; overflow-y: auto">
            ...
            <div class="position-absolute w-100" style="bottom: 8px">
              <div v-if="smsSelected" class="w-100 px-3 d-flex flex-row">
                ...
                <div class="d-flex align-items-center" style="padding-left: 12px; cursor: pointer;"
              ...
              <div v-if="emailSelected" class="w-100 btn px-3 position-relative" style="height: 48px;">
                <div @click="expandCreateEmail = !expandCreateEmail" class="btn btn-primary px-2 py-1 position-absolute"
                  style="right: 16px">Tạo email mới</div>
              </div>
            </div>
    ...
    <modal name="compare-modal">
      <div class="h-100 d-flex flex-column position-relative">
        <div class="modal-title d-flex flex-row align-items-center justify-content-center">
          <div class="col-12 d-flex align-items-center justify-content-center">Xác định mối quan hệ giữa 2 thành viên
          </div>
                ...
                <div class="d-flex flex-column" style="flex-grow: 1; padding-left: 8px;">
                  ...
                  <div class="d-flex flex-row pt-2">
                    <div class="col-6" style="padding-right: 4px">
                      <input type="text" class="compare-modal-item form-control" :value="objCompareMember1.gender"
                        disabled />
                    </div>
                  ...
                  <div class="pt-2">
                    <input type="text" class="compare-modal-item form-control"
                      :value="'Ngày Sinh: ' + (objCompareMember1.dob ? objCompareMember1.dob : 'không rõ')" disabled />
                  </div>
                  <div class="pt-2">
                    <input type="text" class="compare-modal-item form-control"
                      :value="'Ngày Mất: ' + (objCompareMember1.dod ? objCompareMember1.dod : 'không rõ')" disabled />
                  </div>
                </div>
              </div>
            ...
            <div class="h-100 d-flex flex-column p-2" style="background-color: #FFFFFF; border-radius: 0.375rem;">
              <div class="d-flex flex-row pb-2" style="height: 220px">
                ...
                <div class="d-flex flex-column" style="flex-grow: 1; padding-left: 8px;">
                  ...
                  <div class="d-flex flex-row pt-2">
                    <div class="col-6" style="padding-right: 4px">
                      <input type="text" class="compare-modal-item form-control" :value="objCompareMember2.gender"
                        disabled />
                    </div>
                    <div class="col-6" style="padding-left: 4px">
                      <input type="text" class="compare-modal-item form-control"
                  ...
                  <div class="pt-2">
                    <input type="text" class="compare-modal-item form-control"
                      :value="'Ngày Sinh: ' + (objCompareMember2.dob ? objCompareMember2.dob : 'không rõ')" disabled />
                  </div>
                  <div class="pt-2">
                    <input type="text" class="compare-modal-item form-control"
                      :value="'Ngày Mất: ' + (objCompareMember2.dod ? objCompareMember2.dod : 'không rõ')" disabled />
                  </div>
                </div>
              </div>
              <div class="d-flex flex-column" style="flex-grow: 1; background-color: #f2f2f2;">
    ...
    <div>
      <modal name="add-from-list">
        <div class="w-100 h-100 add-head-modal">
          <div class="d-flex flex-row w-100 align-items-center position-relative">
            <div class="col-md-12 modal-title d-flex align-items-center justify-content-center w-100">Thêm {{ TitleModal
            }} Từ Danh Sách</div>
          ...
          <div class="d-flex flex-column" style="height: calc(100% - 50px)">
            ...
            <div class="d-flex flex-column headlist-list-container w-100">
              <table class="table table-member headlist-list m-0">
                ...
                <tbody style="cursor: pointer">
                  <tr v-for="(m, index) in ListUnspecifiedMembers" :key="m.id" @click="selectMemberFromTable(m, index)"
            ...
            <div class="modal-footer">
              <div class="d-flex justify-content-end" style="padding-right: 12px;">
                ...
                <button style="margin-left:10px" type="button" @click="closeModalAddMemberFromList()"
      ...
      <modal name="member-modal">
        <div class="card h-100" v-if="objMemberInfor" style="border: none; position: relative;">
            ...
            <div class="row" style="padding: 0;height: 100%;">
              ...
              <div class="col-10 h-100" style="padding-top: 15px" v-if="extendedInfo">
                <div class="h-100 d-flex flex-row">
                  <div v-if="isEdit" class="col-4" style="padding-right: 8px;height: 50%;">
                    <img style="height:316px;width:100%;margin-bottom:61px" :class="{ fitHeight: heightLarger }"
                      v-if="avatarSrc" :src="avatarSrc" alt="Avatar" @click="triggerFileInputClick()" />
                    ...
                    <div style="margin-bottom:61px; fill: #000000; height: 275px; width: 100%;">
                    ...
                    <div class="form-group" style="display:none">
                      ...
                      <input ref="fileInputRef" type="file" class="form-control input-file" id="imageUpload"
                        accept="image/*" @change="updateAvatar($event)" />
                    </div>
                  </div>
                  <div class="d-flex flex-column" style="flex-grow: 1;">
                    ...
                    <div style="display:flex">
                      ...
                      <div style="position: relative;width: 50%; margin-right: 10px;">
                        <input v-model="objMemberInfor.BirthOrder" type="number" min="0" class="form-control modal-item"
                          placeholder />
                        <label class="form-label-number" for="input" v-if="objMemberInfor.FatherID == null && objMemberInfor.MotherID == null && action != 'AddMarriage' "
                          :class="{ 'active': objMemberInfor.BirthOrder }">Vợ thứ</label>
                            <label class="form-label-number" for="input" v-else
                          :class="{ 'active': objMemberInfor.BirthOrder }">{{ this.action == "AddChild" ? 'Con thứ' :
                            (this.isFather ? "Vợ thứ" : (this.isFather == null ? "Con thứ" : "Chồng thứ")) }}</label>
                      </div>
                    </div>
                    <div style="display:flex">
                      <div style="position: relative; width: 50%;margin-right: 10px;">
                    ...
                    <div class="form-group">
                      <h6 style="margin-bottom:20px; height: 20px">Ngày Sinh (Hệ thống sẽ tự đổi từ ngày dương lịch sang
                        âm lịch và ngược lại)</h6>
                      <div style="display:flex">
                        <div style="position: relative; width: 50%;margin-right: 10px;">
                    ...
                    <div class="form-group" v-if="IsDead == 1">
                      ...
                      <div style="position: relative; margin-right:10px">
                        <input v-model="objMemberInfor.PlaceOfDeadth" type="text" class="form-control modal-item"
                          placeholder />
                        <label class="form-label" for="input">Nơi Mất</label>
                      </div>
                      <div style="position: relative; margin-right:10px">
                        <input v-model="objMemberInfor.GraveSite" type="text" class="form-control modal-item"
                          placeholder />
              ...
              <div class="col-9" style="padding-top: 15px" v-else-if="extendedContact">
                <div class="row">
                  <div style="display:flex">
                    ...
                    <div style="position: relative;width: 50%; margin-right: 10px;">
                      <select v-model="selectDistrictMember" class="form-select modal-item">
                        <option :value="null" selected>Quận/Huyện</option>
                        <option v-for="d in ListDistrictMember" :key="d.id" :value="d.DistrictName">{{ d.DistrictName }}
                        </option>
                      </select>
                  ...
                  <div style="display:flex">
                    <div style="position: relative; width: 50%;margin-right: 10px;">
                      <VuePhoneNumberInput ref="phoneNumberInput" v-model="objMemberContact.Phone" v-bind="props">
                      </VuePhoneNumberInput>
                    </div>
                    <div style="position: relative;width: 50%; margin-right: 10px;">
          ...
          <div class="card-footer modal-footer position-absolute w-100" style="bottom: 0;">
            <div class="d-flex justify-content-end" style="padding-right: 12px;">
              <button v-if="isAdd && memberRole != 3" type="button" class="btn btn-primary mr-2"
                @click="addMember()">Thêm</button>
              <button v-else-if="isEdit && memberRole != 3" type="button" class="btn btn-primary mr-2"
                @click="updateInformation()">Sửa</button>
              <button v-if="nodes.length" style="margin-left:10px" type="button" class="btn btn-danger"
                @click="openCfDelModal('removeMember', null, objMemberInfor.MemberName)">Xóa thành viên</button>
            </div>
          </div>
        </div>
      </modal>
        ...
        <div class="w-100 h-100 add-head-modal">
          ...
          <div class="w-100 d-flex flex-column align-items-center justify-content-center"
            ...
            <div class="d-flex flex-row w-100" style="height: 30%;">
              <div v-if="isRemoveRelationship == 'removeRelationship'"
                class="col-6 d-flex align-items-center justify-content-center">
                <div class="btn text-white" @click="removeRelationship()" style="background-color: rgb(255, 8, 0);">Có
                </div>
              </div>
              <div v-else-if="isRemoveRelationship == 'removeMember'"
                class="col-6 d-flex align-items-center justify-content-center">
                <div class="btn bg-danger text-white" @click="removeMember()">Có</div>
              </div>
              <div v-else-if="isRemoveRelationship == 'LinkRelationship'"
                class="col-6 d-flex align-items-center justify-content-center">
                <div class="btn bg-danger text-white" @click="linkRelationship()">Có</div>
              </div>
              <div v-else-if="isRemoveRelationship == 'SetPaternalAncestor'"
                class="col-6 d-flex align-items-center justify-content-center">
