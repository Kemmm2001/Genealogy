<template>
    <div class="profile-grid-container w-100">
        <div class="profile-sidebar px-3">
            <div @click="selectProfile()" :class="{ chosen: profileSelected }" class="btn w-100 mt-3 text-white"
                style="text-align: start;">Tài khoản cá nhân</div>
            <div @click="selectEditRole()" :class="{ chosen: editRoleSelected }" class="btn w-100 mt-3 text-white"
                style="text-align: start;">Phân quyền trong gia phả
            </div>
            <div @click="selectChangePwd()" :class="{ chosen: changePwdSelected }" class="btn w-100 mt-3 text-white"
                style="text-align: start;">Thay đổi mật khẩu</div>
        </div>
        <div class="profile-content">
            ...
            <div v-if="editRoleSelected" class="content" style="height: 80%;">
                <div class="w-100 h-100 position-relative">
                    <div class="pb-2" style="font-weight: bold;">Tài khoản thành viên
                        thuộc gia phả</div>
                    <div class="family-account">
                        <div v-for="(m, index) in listMemberRole" :key="index" :class="{
                            ...
                            <div class="h-100 w-100"
                                style="display: grid; grid-template-columns: 65% 35%; text-align: center;">
                                <div class="h-100 d-flex align-items-center px-2" style="justify-content: start;">{{ m.Email
                                }}</div>
                                <div class="h-100 d-flex align-items-center p-2" style="justify-content: end;">
                                    <select :disabled="m.RoleID != 1 ? false : true" v-model="m.RoleID"
                                        class="form-select h-100 px-3 py-0" @change="changeRole(m.RoleID, m.AccountID)">
