<template>
  <div class="container-fluid" style="padding-left: 200px; padding-right: 200px">
    ...
    <div class="d-flex flex-column h-100 w-100">
      <div class="d-flex align-items-center w-100 my-2 mx-2" style="height: 48px; font-size: 20px; font-weight: bold;">Tư liệu dòng họ</div>
      ...
      <div class="">
        <table class="table table-article table-articlelist-list m-0">
          ...
          <tbody>
            <tr @click="openEditArticleModal(),getInforArticle(article.ArticleID)" class="articlelist-item articlelist-table-item" v-for="(article,index) in  displayedItems" :key="article.ArticleID">
      ...
      <div class="d-flex flex-row paging justify-content-center m-4" style="position:absolute; bottom: 12px; left: 0; right: 0;">
          ...
          <div class="d-flex flex-row align-items-center">
            <div @click="setCurrentPage(currentPage - 2)" v-if="currentPage > 2" class="page" style="cursor: pointer;">{{ currentPage - 2 }}</div>
            <div @click="setCurrentPage(currentPage - 1)" v-if="currentPage > 1" class="page" style="cursor: pointer;">{{ currentPage - 1 }}</div>
            <div class="page">
              <div :class="{ chosen: true }">{{ currentPage }}</div>
            </div>
            <div @click="setCurrentPage(currentPage + 1)" v-if="Math.ceil(this.articleFilter.length / this.itemsPerPage) > currentPage" class="page" style="cursor: pointer;">{{ currentPage + 1 }}</div>
            <div @click="setCurrentPage(currentPage + 2)" v-if="Math.ceil(this.articleFilter.length / this.itemsPerPage) - 1 > currentPage" class="page" style="cursor: pointer;">{{ currentPage + 2 }}</div>
          </div>
